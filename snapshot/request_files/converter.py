...
def convert(
    file: str, output_as_file: bool, include_suffix: str, output_format: str
) -> None:
    """
...
def convert_req_to_dict(
    ...
    stripped_lines = [
        line.strip() for line in rest_of_content.splitlines() if line.strip()
    ]
...
def convert_req_to_json(
    ...
    return json.dumps(
        convert_req_to_dict(request_file_content, include_extension), indent=4
    )
...
def __adapt_includes(stripped_lines: list, include_extension: str) -> list:
    ...
    for include in __extract_includes(stripped_lines):
        ...
        if filename in intermediate_includes.keys():
            intermediate_includes[filename].append(
    ...
    parsed_includes = [
        ...
        for name, macros in intermediate_includes.items()
    ]
